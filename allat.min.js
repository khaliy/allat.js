/* (c) 2013 MIT license author: tszebeni.hu 2013/08/24 05:16 PM */
...
return module;}
ModuleSupport.inject=inject;function define(name,factory,dependencies,singleton){if(!ModuleSupport._modules[name]){ModuleSupport._modules[name]={build:function(){var module=factory(defaultModuleFactory(name));return inject(module,dependencies);},singleton:singleton};}}
...
util.proxy=proxy;Allat.util=util;}(Allat));(function(Allat){var $,prevHandler;if(root.jQuery){$=root.jQuery;}else{$=function(callback){prevHandler=root.onload;root.onload=function(e){if(prevHandler){prevHandler(e);}
if(callback){callback(e);}};};}
Allat.module.define("BootModule",function(BootModule){BootModule.init=function(){$(Allat.util.proxy(function(){this.services.EventBus.fireEvent("load");},this));};return BootModule;},["EventBus"],true);Allat.module.instantiate("BootModule");}(Allat));(function(Allat){Allat.module.define("EventBus",function(EventBus){EventBus.init=function(){this._subscribers=this.services.MapSupport.createMap();};EventBus.subscribe=function(event,handler){this._subscribers.add(event,handler);};EventBus.unSubscribe=function(event,handler){this._subscribers.remove(event,handler);};EventBus.fireEvent=function(event,data){var handlers=this._subscribers.getValues(event),i;if(!handlers||!handlers.length){return;}
for(i=0;i<handlers.length;i++){if(handlers[i](event,data)){break;}}};return EventBus;},["MapSupport"],true);}(Allat));(function(Allat){Allat.module.define("MapSupport",function(MapSupport){var Map=function(){this._map={};};Map.prototype.add=function(key,value){var values=this._map[key]||[];values.push(value);this._map[key]=values;};Map.prototype.remove=function(key,value){var values=this._map[key],i;for(i=0;i<values.length;i++){if(value===values[i]){this._map[key]=values.splice(i,1);break;}}
...
return false;};Map.prototype.getValues=function(key){return this._map[key];};Map.prototype.getKeys=function(){return Object.getKeys(this._map);};MapSupport.createMap=function(){return new Map();};return MapSupport;},[],true);}(Allat));(function(Allat){Allat.module.define("PropertyBindSupport",function(PropertyBindSupport){PropertyBindSupport.init=function(){var template,i,boundTo,key,p=Allat.util.proxy,createChangeHandler=p(function(key,template){return p(function(e){var value=e.target.value;this.dictionary[key]={template:template,boundTo:e.target,value:value};template.innerHTML=value;this.services.EventBus.fireEvent("property.updated",{key:key,value:value});},this);},this);this.dictionary={};this.templates=document.getElementsByClassName("bind");for(i=0;i<this.templates.length;i++){template=this.templates[i];if(template.attributes["data-bind"]){key=template.attributes["data-bind"].value;boundTo=document.getElementById(key);boundTo.onchange=createChangeHandler(key,template);boundTo.onkeyup=boundTo.onchange;boundTo.onchange({target:boundTo});}}
this.services.EventBus.subscribe("property.set",p(function(event,data){if(this.resolve(data.key)){this.update(data.key,data.value);}},this));};PropertyBindSupport.resolve=function(key){return this.dictionary[key].value;};PropertyBindSupport.update=function(key,value){this.dictionary[key].value=value;this.dictionary[key].boundTo.value=value;this.dictionary[key].boundTo.onchange({target:this.dictionary[key].boundTo});return value;};return PropertyBindSupport;},["EventBus"],true);}(Allat));(function(Allat){Allat.module.define("TemplateSupport",function(TemplateSupport){function addSimpleTemplate(){var SimpleTemplate={render:function(template,data){var keys=Object.getKeys(data),l=keys.length,i,regex;for(i=0;i<l;i++){regex=new RegExp("{"+keys[i]+"}","g");template=template.replace(regex,data[keys[i]]);}
